...
class TestDex(unittest.TestCase):
    ...
    class AddLiquidity(unittest.TestCase):
        ...
        def test01_it_adds_liquidity_successfuly(self):
            ...
            factory, _, _, _ = Env().deploy_full_app(DEFAULT_BAKER)
            ...
            token_a.approve({"spender": factory.address, "value": amount_a}).send(
                **send_conf
            )
            token_b.mint({"address": ALICE_PK, "value": amount_b}
                         ).send(**send_conf)
            token_b.approve({"spender": factory.address, "value": amount_b}).send(
                **send_conf
            )
            ...
            token_a.mint({"address": ALICE_PK, "value": add_amount_a}).send(
                **send_conf)
            token_a.approve({"spender": dex.address, "value": add_amount_a}).send(
                **send_conf
            )
            token_b.mint(
                {
                    "address": ALICE_PK,
                    "value": add_liquidity_params["max_tokens_deposited"],
                }
            ).send(**send_conf)
            token_b.approve(
                {
            ...
            self.assertEqual(
                dex.storage["lqt_total"](),
                lqt_total + add_amount_a * lqt_total // amount_a,
            )
            self.assertEqual(
                dex.storage["token_pool_a"](), add_amount_a + amount_a)
            self.assertEqual(
                dex.storage["token_pool_b"](),
                amount_b + add_amount_a * amount_b // amount_a,
            )
            self.assertEqual(
                lqt.storage["tokens"][ALICE_PK](),
                alice_liquidity + add_amount_a * lqt_total // amount_a,
            )

            TestDex.print_success("test1_it_adds_liquidity_successfuly")

        def test02_it_adds_liquidity_with_xtz_successfully(self):
            TestDex.print_start(
            ...
            factory.launchExchange(launch_exchange_params).with_amount(amount_a).send(
                **send_conf
            )
            dex_address = factory.storage["pairs"][
            ...
            dex.addLiquidity(add_liquidity_params).with_amount(add_amount_a).send(
                **send_conf
            )
            self.assertEqual(dex.context.get_balance(),
                             amount_a + add_amount_a)
            self.assertEqual(
                dex.storage["token_pool_a"](), amount_a + add_amount_a)
        ...
        def test03_it_fails_when_max_tokens_deposited_is_less_than_tokens_deposited(
            self,
        ):
            TestDex.print_start(
            ...
            with self.assertRaises(MichelsonError) as err:
                add_liquidity_params["max_tokens_deposited"] = (
                    add_amount_a * amount_b // amount_a - 1
                )
                dex.addLiquidity(add_liquidity_params).send(**send_conf)
        ...
        def test04_it_fails_when_lqt_minted_is_less_than_min_lqt_minted(self):
            TestDex.print_start(
            ...
            with self.assertRaises(MichelsonError) as err:
                add_liquidity_params["min_lqt_minted"] = (
                    add_amount_a * lqt_total // amount_a + 1
                )
                dex.addLiquidity(add_liquidity_params).send(**send_conf)
        ...
        def test05_it_fails_when_incorrect_xtz_amount_is_sent(self):
            TestDex.print_start(
    ...
    class RemoveLiquidity(unittest.TestCase):
        def test0_before_tests(self):
            TestDex.print_title(self)

        def test01_it_removes_liquidity_successfuly(self):
            ...
            TestDex.print_success("test1_it_removes_liquidity_successfuly")

        def test02_it_removes_liquidity_with_baker_rewards_update(self):
            TestDex.print_start(
            ...
            dex.removeLiquidity(remove_liquidity_params).send(**send_conf)
            self.assertEqual(
                dex.storage["user_rewards"][ALICE_PK](
        ...
        def test03_it_fails_when_token_a_withdrawn_is_less_than_min_tokens_withdrawn(
            self,
        ):
            TestDex.print_start(
            ...
            with self.assertRaises(MichelsonError) as err:
                remove_liquidity_params["min_token_a_withdrawn"] = (
                    remove_amount * amount_a // lqt_total + 1
                )
                dex.removeLiquidity(remove_liquidity_params).send(**send_conf)
        ...
        def test04_it_fails_when_token_b_withdrawn_is_lesser_than_min_tokens_withdrawn(
            self,
        ):
            TestDex.print_start(
            ...
            with self.assertRaises(MichelsonError) as err:
                remove_liquidity_params["min_token_b_withdrawn"] = (
                    remove_amount * amount_b // lqt_total + 1
                )
                dex.removeLiquidity(remove_liquidity_params).send(**send_conf)
        ...
        def test05_it_fails_when_burn_amount_is_greater_than_the_total_lqt(self):
            TestDex.print_start(
    ...
    class Swap(unittest.TestCase):
        ...
        def test01_it_swaps_successfully_fa12_xtz(self):
            ...
            factory.launchExchange(launch_exchange_params).with_amount(amount_b).send(
                **send_conf
            )
            dex_address = factory.storage["pairs"][
            ...
            dex = pytezos.contract(dex_address).using(**_using_params)
            tokens_sold = 10**4
            token_a.mint({"address": ALICE_PK, "value": tokens_sold}).send(
                **send_conf)
            token_a.approve({"spender": dex_address, "value": tokens_sold}).send(
                **send_conf
            )
            ...
            self.assertEqual(
                sink.storage["reserve"][{
                    "fa12": token_a.address}](), reserve_amount
            )

            self.assertEqual(
                dex.storage["token_pool_a"](),
                amount_a + tokens_sold - (burn_amount + reserve_amount),
            )
            self.assertEqual(dex.storage["token_pool_b"](), amount_a - bought)
            self.assertEqual(
                token_a.storage["tokens"][dex_address](),
                amount_a + tokens_sold - (burn_amount + reserve_amount),
            )
            TestDex.print_success("test1_it_swaps_successfully_fa12_xtz")

        def test02_it_swaps_successfully_xtz_fa12(self):
            ...
            token_b.approve({"spender": factory.address, "value": amount_a}).send(
                **send_conf
            )
            factory.launchExchange(launch_exchange_params).with_amount(amount_a).send(
                **send_conf
            ...
            self.assertEqual(
                int(internal_operations[0]["amount"]
                    ), burn_amount + reserve_amount
            )

            self.assertEqual(sink.storage["burn"]
                             [{"xtz": None}](), burn_amount)
            self.assertEqual(sink.storage["reserve"]
                             [{"xtz": None}](), reserve_amount)

            self.assertEqual(
                dex.storage["token_pool_a"](),
                amount_a + tokens_sold - (burn_amount + reserve_amount),
            )
            self.assertEqual(dex.storage["token_pool_b"](), amount_b - bought)
            self.assertEqual(
                token_b.storage["tokens"][dex_address](), amount_b - bought
            )
            TestDex.print_success("test2_it_swaps_successfully_xtz_fa12")

        def test03_it_swaps_successfully_fa2_xtz(self):
            ...
            token_a.mint(
                {"address": ALICE_PK, "amount": amount_a,
                    "metadata": {}, "token_id": 0}
            ).send(**send_conf)
            ...
            self.assertEqual(
                sink.storage["burn"][{
                    "fa2": (token_a.address, 0)}](), burn_amount
            )
            self.assertEqual(
                sink.storage["reserve"][{
                    "fa2": (token_a.address, 0)}](), reserve_amount
            )
            ...
            self.assertEqual(dex.storage["token_pool_b"](), amount_a - bought)
            self.assertEqual(
                token_a.storage["ledger"][(dex_address, 0)](),
                amount_a + tokens_sold - (burn_amount + reserve_amount),
            )
            TestDex.print_success("test3_it_swaps_successfully_fa2_xtz")

        def test04_it_swaps_successfully_xtz_fa2(self):
            ...
            token_b.mint(
                {"address": ALICE_PK, "amount": amount_b,
                    "metadata": {}, "token_id": 0}
            ).send(**send_conf)
            ...
            self.assertEqual(dex.storage["token_pool_b"](), amount_b - bought)
            self.assertEqual(
                token_b.storage["ledger"][(
                    dex_address, 0)](), amount_b - bought
            )
            TestDex.print_success("test4_it_swaps_successfully_xtz_fa2")

        def test05_it_swaps_successfully_fa12_fa2(self):
            ...
            TestDex.print_success("test5_it_swaps_successfully_fa12_fa2")

        def test06_it_swaps_successfully_fa2_fa12(self):
            ...
            TestDex.print_success("test6_it_swaps_successfully_fa2_fa12")

        def test07_it_swaps_successfully_fa12_fa12(self):
            ...
            TestDex.print_success("test7_it_swaps_successfully_fa12_fa12")

        def test08_it_swaps_successfully_fa2_fa2(self):
            ...
            TestDex.print_success("test8_it_swaps_successfully_fa2_fa2")

        def test09_it_swaps_successfully_fa12_fa12_flat(self):
            ...
            fun = abs((amount_a + amount_b) ** 8) - \
                ((amount_a - amount_b) ** 8)
            # for pegged tokens with pool sizes 10 ** 6
            # and tokens_in = 10 ** 4 * 9990 // 10 ** 4 = 9990,
            # bought is 9989
            bought = newton(amount_a, amount_b, tokens_in, 0, fun, 5)
            swap_params = {
                "t2t_to": ALICE_PK,
                "tokens_sold": tokens_sold,
                "min_tokens_bought": bought,
            ...
            TestDex.print_success("test9_it_swaps_successfully_fa12_fa12_flat")
        ...
        def skip_test10_it_compares_curve_swaps_fa12_fa12_flat(self):
            ...
            for num in range(20):
                ...
                amount_a = 10**12
                pool_multiplier = (num + 1) / 10 if num < 10 else (num - 8)
                amount_b = (
                    amount_a *
                    (num + 1) // 10 if num < 10 else amount_a * (num - 8)
                )
                token_a.mint({"address": ALICE_PK, "value": 2 * amount_a}).send(
                    **send_conf
                )
                token_a.approve(
                    {"spender": factory.address, "value": 2 * amount_a}
                ).send(**send_conf)
                token_b.mint({"address": ALICE_PK, "value": amount_b}).send(
                    **send_conf)
                token_b.approve({"spender": factory.address, "value": amount_b}).send(
                    **send_conf
                )
                token_c.mint({"address": ALICE_PK, "value": amount_b}).send(
                    **send_conf)
                token_c.approve({"spender": factory.address, "value": amount_b}).send(
                    **send_conf
                )
                ...
                tokens_sold = 10**10
                token_a.mint({"address": ALICE_PK, "value": 2 * tokens_sold}).send(
                    **send_conf
                )
                token_a.approve({"spender": dex_1_address, "value": tokens_sold}).send(
                    **send_conf
                )
                swap_params = {
                    "t2t_to": BOB_PK,
                ...
                token_a.approve({"spender": dex_2_address, "value": tokens_sold}).send(
                    **send_conf
                )
                ...
                with open(
                    "../../compare_curve.txt", "a", encoding="UTF-8"
                ) as output_file:
                    output_file.write(
                        f"token_pool_a : {amount_a} tokens; token_pool_b : {amount_b} tokens; \n"
                    )
                    output_file.write(f"pool ratio : {pool_ratio} \n")
                    output_file.write(
    ...
    class ClaimReward(unittest.TestCase):
        def test0_before_tests(self):
            TestDex.print_title(self)

        def test01_it_claims_rewards_successfully_one_deposit(self):
            TestDex.print_start(
            ...
            pytezos.transaction(destination=dex.address, amount=added_amount).send(
                **send_conf
            )
            ...
            self.assertEqual(
                alice_reward_paid,
                dex.storage["user_rewards"][ALICE_PK]["reward_paid"](),
            )

            internal_operations = resp.opg_result["contents"][0]["metadata"][
                "internal_operation_results"
            ]
            self.assertEqual(internal_operations[0]["destination"], ALICE_PK)
            self.assertGreaterEqual(
        ...
        def test02_it_claims_reward_successfully_multiple_deposits(self):
            TestDex.print_start(
            ...
            self.assertEqual(dex_balance, amount_a + added_amount * num)

            self.assertEqual(dex.storage["total_reward"](), added_amount * num)

            self.assertLessEqual(
                dex.storage["reward_per_share"](),
                added_amount * num * ACCURANCY_MULTIPLIER // amount_a,
            )

            self.assertGreaterEqual(
                dex.storage["reward_per_share"](),
                added_amount * num * ACCURANCY_MULTIPLIER // amount_a - 1,
            )
            ...
            self.assertEqual(dex_balance, amount_a + num * added_amount)
            ...
            self.assertEqual(internal_operations[0]["destination"], ALICE_PK)
            self.assertGreaterEqual(
                int(internal_operations[0]["amount"]), added_amount * num - 1
            )

            self.assertLessEqual(
                int(internal_operations[0]["amount"]), added_amount * num
            )
        ...
        def test03_it_claims_reward_successfully_two_lps(self):
            ...
            dex.addLiquidity(add_liquidity_params).with_amount(amount_a).send(
                **send_conf
            )
            ...
            self.assertEqual(
                total_lqt, sqrt(amount_a * amount_b) +
            ...
            self.assertLessEqual(
                reward_per_share - 1, dex.storage["reward_per_share"]()
            )
            ...
            resp = (
                bob_pytezos.contract(dex.address)
                .claimReward(ALICE_PK)
                .send(**send_conf)
            )
    ...
    class Default(unittest.TestCase):
        ...
        def test01_it_deposits_xtz(self):
            ...
            dex = pytezos.contract(dex_address).using(**_using_params)
            deposit = 10**4
            pytezos.transaction(destination=dex_address, amount=deposit).send(
                **send_conf
            )
            ...
            lqt_total = dex.storage["lqt_total"]()
            rewards_time = (
                period_finish if pytezos.now() > period_finish else pytezos.now()
            )
            ...
            TestDex.print_success("test1_it_deposits_xtz")

        def test02_it_fails_if_no_pool_is_xtz(self):
            TestDex.print_start("test1_it_deposits_xtz")
            factory, _, _, _ = Env().deploy_full_app(DEFAULT_BAKER)
            token_a = Env().deploy_fa2(FA2Storage())
            token_b = Env().deploy_fa2(FA2Storage())
