let remove_admin (param : address) (store : storage) : return =
    if Tezos.sender <> Tezos.self_address then
        ...
        let func () = 
            let remove_admin_entrypoint = 
                Option.unopt (Tezos.get_entrypoint_opt "%removeAdmin" sender_address : address contract option) in
            [Tezos.transaction param 0mutez remove_admin_entrypoint] in
