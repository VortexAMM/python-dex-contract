...
from test_env import Env, ALICE_PK, pytezos, FA12Storage, FA2Storage, FactoryStorage, LqtStorage, send_conf, _using_params, bob_pytezos, BOB_PK
    ...
    def test_claim_multiple(self):
        ...
        for i in range(1):
            ...
            for j in range(4):
                ...
                token_a.mint({"address": ALICE_PK, "amount": 10 ** 10,
                ...
                token_b.mint({"address": ALICE_PK, "amount": 10 ** 10,
                              "metadata": {}, "token_id": 0}).send(**send_conf)
                ...
                dex = factory.storage["pairs"][(token_a_type, token_b_type)]()
        ...
        for token in tokens:
            ...
            smak_token.approve({"spender": factory.address,
        ...
        for pair in pairs:
            ...
            token_a = pytezos.contract(token_a_address).using(**_using_params)
            token_b = pytezos.contract(token_b_address).using(**_using_params)

            dex_addr = factory.storage["pairs"][pair]()
            token_a.mint({"address": ALICE_PK, "amount": 2 * 10 ** 5,
                          "metadata": {}, "token_id": 0}).send(**send_conf)
            token_a.update_operators([{"add_operator": {
                "owner": ALICE_PK, "operator": dex_addr, "token_id": 0}}]).send(**send_conf)
            token_b.mint({"address": ALICE_PK, "amount": 2 * 10 ** 5,
                          "metadata": {}, "token_id": 0}).send(**send_conf)
            token_b.update_operators([{"add_operator": {
                "owner": ALICE_PK, "operator": dex_addr, "token_id": 0}}]).send(**send_conf)
            ...
            self.assertEqual(sink.storage["burn"][token_a_type](), burn_amount)
            self.assertEqual(sink.storage["reserve"]
                             [token_a_type](), reserve_amount)
            ...
            self.assertEqual(sink.storage["burn"][token_b_type](), burn_amount)
            self.assertEqual(sink.storage["reserve"]
                             [token_b_type](), reserve_amount)

        opg = bob_pytezos.contract(sink.address).claim({
            "tokens": tokens,
        ...
        with open('claim_gas_fa2.txt', 'a') as f:
            f.write(
                f'{tokens_length} tokens claimed; {consumed_gas} gas; {fee} mutez fee; \n')

    def test_claim_multiple_fa12(self):
        factory, smak_token, sink = Env().deploy_full_app()
        pairs = []
        dexs = []
        tokens = []
        ...
        for i in range(1):
            ...
            for pair in pairs:
                ...
                dex_addr = factory.storage["pairs"][pair]()
                token_a.mint(
                    {"address": ALICE_PK, "value": 2 * 10 ** 5}).send(**send_conf)
                token_a.approve({"spender": dex_addr,
                                 "value": 2 * 10 ** 5}).send(**send_conf)
                token_b.mint(
                    {"address": ALICE_PK, "value": 2 * 10 ** 5}).send(**send_conf)
                token_b.approve({"spender": dex_addr,
                                 "value": 2 * 10 ** 5}).send(**send_conf)
                dex = pytezos.contract(dex_addr).using(**_using_params)
                dex.swap({
                    "t2t_to": ALICE_PK,
            ...
            fee = opg.opg_result["contents"][0]["fee"]
            tokens_length = len(tokens)
            with open('analize_claim_gas_fa12.txt', 'a') as f:
                f.write(
                    f'{tokens_length} tokens claimed; {consumed_gas} gas; {fee} mutez fee; \n\n')

    def test_sink_claim_reward(self):
        factory, smak_token, sink = Env().deploy_full_app()
        token_a = Env().deploy_fa12(FA12Storage())
        token_b = Env().deploy_fa12(FA12Storage())
        token_a.mint(
            {"address": ALICE_PK, "value": 2 * 10 ** 12 + 10 ** 10}).send(**send_conf)
        token_a.approve({"spender": factory.address,
                         "value": 2 * 10 ** 12}).send(**send_conf)
        token_b.mint(
            {"address": ALICE_PK, "value": 10 ** 12}).send(**send_conf)
        token_b.approve({"spender": factory.address,
                         "value": 10 ** 12}).send(**send_conf)
        smak_token.mint(
            {"address": ALICE_PK, "value": 10 ** 12}).send(**send_conf)
        smak_token.approve({"spender": factory.address,
                            "value": 10 ** 12}).send(**send_conf)
        ...
        factory.launchExchange(launch_exchange_params).send(**send_conf)
        dex_addr = factory.storage["pairs"][({"fa12": token_a.address}, {
        ...
        reserve_address = dex.storage["reserve"]()
        ...
        reward_rate = dex.storage["reward_rate"]()
        self.assertEqual(smak_token.storage["tokens"][BOB_PK](
        ), bought * reward_rate // 10 ** 4)
        self.assertEqual(token_a.storage["tokens"]
