...
from env import Env, ALICE_PK, pytezos, FA12Storage, FA2Storage, send_conf, _using_params, bob_pytezos, BOB_PK, metadata, token_metadata
import unittest
...
class TestSink(unittest.TestCase):
    class Claim(unittest.TestCase):
        ...
        def test07_it_fails_when_token_to_burn_not_listed(self):
            ...
            token_b = Env().deploy_fa12(FA12Storage())
            token_a.mint(
                {"address": ALICE_PK, "value": 10 ** 12 + 10 ** 10}).send(**send_conf)
            token_a.approve({"spender": factory.address,
                             "value": 2 * 10 ** 12}).send(**send_conf)
            token_b.mint(
                {"address": ALICE_PK, "value": 2 * 10 ** 12}).send(**send_conf)
            token_b.approve({"spender": factory.address,
                             "value": 2 * 10 ** 12}).send(**send_conf)
            smak_token.mint(
                {"address": ALICE_PK, "value": 10 ** 12}).send(**send_conf)
